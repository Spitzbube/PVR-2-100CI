...
//   FAPI_SYS_PRINT_MSG("AVINT_VideoInit\n");
   ...
   struct av_72740* pVideo = &av->Data_72740;
...
int av_init(void)
{
   ...
   struct appData* fp = func_21b8e50c();
...
//   printf("func_21b7a764\n");
   ...
   if (av->Data_4232 != 0)
   {
      for (i = 0; (char)i < func_21b8e50c()->bData_0; i++)
      {
...
int av_open_tuner(void)
{
   ...
   //21b7ab70
   if (0 == func_21b98284(&sp36)) //settings_get_video_output?
   {
      //21b7ab8c
      func_21b9976c(sp36); //settings_set_video_output
   }
...
int av_display_iframe(uint32_t* dataPtr, uint32_t sizeInBytes)
{
   VAL_OpenParamsT sp;
   int32_t res = 0;
   struct av_72740* r4 = &av->Data_72740;
...
int32_t AV_PowerupScreenShow(int openVideo)
{
   int sp;
   int res = 0;
   struct appData* sysCfg = func_21b8e50c();

   //FAPI_SYS_PRINT_MSG("21b79318\n");

   if ((av->Data_72740.bootScreenActive == 0) &&
         (sysCfg->bootScreen != 0))
   {
      if (openVideo != 0)
      {
...
int AVINT_VideoOpen(void)
{
   ...
   FAPI_VIENC_EnableDisableWss(
         VAL_GetViencSd(av->Data_72740.valDataPtr),
...
//   printf("av_get_vienc_hd");
   ...
   if (av->Data_72740.valDataPtr != 0)
   {
      return VAL_GetViencHd(av->Data_72740.valDataPtr);
   }
