...
static inline void cal_video_pts(unsigned int*pts, unsigned int ch)
{
    *pts = video_pts[ch];
    switch(streams[ch].streamFormat.fps)
    {
        ...
        default:
            video_pts[ch] += (90000 / streams[ch].streamFormat.fps);    
    }
...
static void read_video_data(void *argc)
{
    ...
    while(read_video_loop) {
        ...
        if(encodingState){
            ...
            cal_video_pts(&(header.pts), stream.stream_id);

            header.frame_no = video_frame_index[stream.stream_id] ++;
            if(video_writer[stream.stream_id] != NULL)
                mediabuf_write_frame(video_writer[stream.stream_id], stream.addr, stream.size, &header);
