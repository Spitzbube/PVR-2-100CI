...
void osd_draw_image(void)
{
    GADI_OSD_AreaParamsT areaParams;
    GADI_OSD_BitMapAttrT bitMapAttr;
    GADI_U32 bitMapSize = 3*bitMapAttr.height*bitMapAttr.width;

    gadi_sys_memset(&areaParams, 0, sizeof(GADI_OSD_AreaParamsT));

    int fd = open("/etc/videoconfig/gokelogo.bmp", O_RDONLY);
    if (fd < 0) {
        GADI_ERROR("Open logo bitmap file failed\n");
        return ;
    }

    bitMapAttr.bitMapColorFmt = GADI_OSD_COLORFMT_RGB888;
    bitMapAttr.alpha = 255;//no transparent.
    bitMapAttr.height = 64;
    bitMapAttr.width = 64;
    bitMapAttr.pitch = bitMapAttr.width*3;//no 4 byte align
    bitMapSize = 3*bitMapAttr.height*bitMapAttr.width;
    bitMapAttr.bitMapAddr = malloc(bitMapSize);
    memset(bitMapAttr.bitMapAddr, 0, bitMapSize);

    lseek(fd, 54, SEEK_SET);//jump read bitmap header
    read(fd, bitMapAttr.bitMapAddr, bitMapSize);
    close(fd);

    areaParams.enable  = 1;
    areaParams.areaId = 0;
    areaParams.planeId = 0;
    areaParams.offsetX = 8;
    areaParams.offsetY = 8;
...
static int handle_osd_command(int argc, char* argv[])
{
    ...
    if(showPicFlag == 1)
    {
        retVal = osd_function_show_bmp(osdHandle, planeId, areaId, &bmpPars);
        if(retVal != 0)
        {
            printf("show picture error.");
        }
        gadi_sys_free(bmpPars.bmpFile);
        bmpPars.bmpFile = NULL;
        showPicFlag = 0;
