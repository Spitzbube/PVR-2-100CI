...
#else
	if (chip->info->ecclayout->oobavail < sizeof(struct yaffs_packed_tags2))
#endif		
...
int spinand_yaffs_devconfig(char *_mp, int start_block, int end_block)
{
	struct mtd_info *mtd = NULL;
	struct yaffs_dev *dev = NULL;
	struct yaffs_dev *chk;
...
int spinand_yaffs_mount(char *mp)
{
	int retval = yaffs_mount(mp);
	if (retval < 0){
		printf("Error mounting %s, return value: %d, %s\n", mp,
...
void spinand_yaffs_umount(char *mp)
{
	if (yaffs_unmount(mp) == -1)
		printf("Error umounting %s, return value: %d, %s\n", mp,
			yaffsfs_GetError(), yaffs_error_str());
...
int spinand_yaffs_open_file(char *fn)
{
	int h;
...
void spinand_yaffs_close_file(int handle)
{
	yaffs_close(handle);
}
#endif
