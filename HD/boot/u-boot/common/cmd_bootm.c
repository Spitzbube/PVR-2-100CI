...
#ifdef CONFIG_UBOOT_TO_KERNEL
	puts("put param to memory\n");
#if 0

	s = getenv ("soctype");
    str_val = s ? (int)simple_strtol(s, NULL, 10) : CONFIG_SOC_TYPE;
	*(volatile u32 *) (CONFIG_U2K_SOC_ADDR) = str_val;
#else
	*(volatile u32 *) (CONFIG_U2K_SOC_ADDR) = 0;
#endif
    s = getenv ("mem");
    str_val = s ? (int)simple_strtol(s, NULL, 10) : CONFIG_KERNEL_MEM_SIZE;
	*(volatile u32 *) (CONFIG_U2K_MEM_ADDR) = str_val*1024*1024;
	printf("mem size (%d)\n",str_val);

#if 0
	s = getenv ("totalmem");
    str_val = s ? (int)simple_strtol(s, NULL, 10) : CONFIG_USR_TOTAL_MEM_SIZE;
	*(volatile u32 *) (CONFIG_U2K_TOTAL_MEM) = str_val;
#else
	*(volatile u32 *) (CONFIG_U2K_TOTAL_MEM) = (gd->ram_size)>>20;
#endif
	printf("total mem size (%d)\n",(gd->ram_size)>>20);

	s = getenv ("bsbsize");
    str_val = s ? (int)simple_strtol(s, NULL, 10) : CONFIG_BSB_MEM_SIZE;
	*(volatile u32 *) (CONFIG_U2K_BSB_ADDR) = str_val*1024*1024;
	printf("bsb size (%d)\n",str_val);

	s = getenv ("usrsize");
	str_val = s ? (int)simple_strtol(s, NULL, 10) : CONFIG_USR_MEM_SIZE;
	*(volatile u32 *) (CONFIG_U2K_USR_ADDR) = str_val*1024*1024;
	printf("usr size (%d)\n",str_val);

	*(volatile u32 *) (CONFIG_U2K_ENABLE) = 0x7101;

    s = getenv ("phytype");
    str_val = s ? (int)simple_strtol(s, NULL, 10) : 0;
    *(volatile u32 *) (CONFIG_U2K_PHY_TYPE) = str_val;

    s = getenv ("arm_freq");
    str_val = s ? getenv_ulong("arm_freq", 16, str_val) : 0x00112032;
    *(volatile u32 *) (CONFIG_U2K_ARM_FREQ) = str_val;
#endif
...
#endif
	default:	
#if defined(CONFIG_GK6202_EVB) || defined(CONFIG_GK7101) || defined(CONFIG_GK7101S)
