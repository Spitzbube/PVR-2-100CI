...
extern int gk7101_gpio_func_config(u32 gpioid, u32 gtype);
...
int crc_check(char *buf,unsigned int size,unsigned int pkt_crc)
{
    ...
    //printf("crc_ret = 0x%x  pkt_crc = 0x%x\n",crc,pkt_crc);
    if(crc != pkt_crc)
        return -1;
    return 0;
}
...
int get_partition_inf(char *buf,unsigned int size,pkt_cfg *pkt_info_cfg)
{
    ...
    if(pkt_info_cfg->update_zone < 0 || pkt_info_cfg->update_zone > (2^32))
    {
...
int do_gkupdate ( cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
        ...
        {
            char *cfg_file =   {"gk7101-evb_image_sd_update.cfg"};
            char *image_file = {"gk7101-evb_image.bin"};
            cmd_tbl_t * fat_cmdtp = NULL;   
            int fat_flag = 1;
            int fat_argc = 2;
            char *fat_argv []={"fatls","mmc","0:0"}; 
            do_fat_ls(fat_cmdtp,fat_flag,fat_argc,fat_argv);
            
            char *cfg_prt    = (char *)CFG_MEM_ADDR;
            cfg_size = file_fat_read_at(cfg_file, 0, cfg_prt, 0);
            if(cfg_size <= 0)
            {
                printf("bad gk7101-evb_image_sd_update.cfg,exit update from sd card\n");
                return -1;
            }
            memset(&pkt_info_cfg,0,sizeof(pkt_cfg));
            if (0 != get_partition_inf(cfg_prt,cfg_size,&pkt_info_cfg))
            {
                printf("exit gkupdate from sd card cfg_size = %d\n",cfg_size);
                return -1;
            }
            ...
            if(ptk_size <= 0)
            {
                printf("bad gk7101-evb_image.bin,exit update from sd card\n");
                return -1;
            }
