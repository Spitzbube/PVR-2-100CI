...
#define GK6202_USB_OFFSET       2
static inline u8 musb_readb(const void *addr)
{
	volatile u32 usb_mode = 0;
	u32 offset;

	offset = (u32)(addr - (void*)musbr);
	addr = (void*)musbr + (offset << GK6202_USB_OFFSET);
	
	usb_mode = readl(GK6202_USB1_MODE);
	usb_mode &= ~GK6202_USB_LENGTH_MASK;
  	writel(GK6202_USB_1BYTE_ACCESS<<1 | usb_mode, GK6202_USB1_MODE);
  	return __raw_readb(addr);
}

static inline void musb_writeb(u8 data,void *addr)
{
	volatile u32 usb_mode = 0;
	u32 offset;
...
static inline u16 musb_readw(const void *addr)
{
	u16 data;
   	volatile u32 usb_mode = 0;
	u32 offset;
	...
	addr = (void*)musbr + (offset << GK6202_USB_OFFSET);
  	...
  	usb_mode = readl(GK6202_USB1_MODE);
  	usb_mode &= ~GK6202_USB_LENGTH_MASK;
   	writel(GK6202_USB_2BYTE_ACCESS<<1 | usb_mode, GK6202_USB1_MODE);
   	data = __raw_readw(addr);
   	return ( data );
}

static inline void musb_writew(u16 data,void *addr)
{
	volatile u32 usb_mode = 0;
	u32 offset;
...
static inline u32 musb_readl(const void *addr)
{
	u32 data;
   	volatile u32 usb_mode = 0;
	u32 offset;
	...
	addr = (void*)musbr + (offset << GK6202_USB_OFFSET);
  	...
  	usb_mode = readl(GK6202_USB1_MODE);
   	usb_mode &= ~GK6202_USB_LENGTH_MASK;
   	writel(GK6202_USB_4BYTE_ACCESS<<1 | usb_mode, GK6202_USB1_MODE);
   	data = __raw_readl(addr);
  	return ( data );
}

static inline void musb_writel(u32 data,void *addr)
{
	volatile u32 usb_mode = 0;
	u32 offset;
...
static inline u8 musb_read_ulpi_buscontrol(struct musb_regs *musbr)
{
	return musb_readb(&musbr->ulpi_busctl);
}
static inline void musb_write_ulpi_buscontrol(struct musb_regs *musbr, u8 val)
{
	musb_writeb(val, &musbr->ulpi_busctl);
}
#endif
