/*
 * GK7101 SpiNAND driver at U-Boot
 *
...
static struct nand_ecclayout spinand_ecc_64_layout_wb = {
    ...
    .oobfree    = {
                ...
                {.offset = 0x12, .length = 6},
                {.offset = 0x22, .length = 6},
                {.offset = 0x32, .length = 6},
    }
};
...
static int gk7101_spinand_set_channel(struct spinand_info *info)
{
...
static int gk7101_spinand_release_channel(struct spinand_info *info)
{
    uint32_t ce_setting     = 0;
    uint32_t new_ce_setting = 0;
...
/**
 * gk7101_spinand_write_enable - send command 0x06 to enable write or erase the Nand cells
 *
 * Description:
 ...
 */
static uint32_t gk7101_spinand_write_enable(struct spinand_chip *chip, int enable)
{
...
/**
 * gk7101_spinand_set_feature - send command 0x1f to the SPI Nand status register
 *
 * Description:
 ...
 */
static int gk7101_spinand_set_feature(struct spinand_chip *chip, uint32_t off, uint8_t input)
{
    struct spinand_info *info = chip->info;
...
/**
 * gk7101_spinand_get_feature - send command 0xf to the SPI Nand status register
 *
 * Description:
 *    After read, write, or erase, the Nand device is expected to set the busy status.
 *    This function is to allow reading the status of the command: read, write, and erase.
 *    Once the status turns to be ready, the other status bits also are valid status bits.
 */
static int gk7101_spinand_get_feature(struct spinand_chip *chip, uint32_t off, uint32_t *output)
{
    struct spinand_info *info = chip->info;
 ...
 */
static int gk7101_spinand_read_id(struct spinand_chip *chip, uint32_t* id)
{
    struct spinand_info *info = chip->info;
    ...
    gk7101_spinand_set_channel(info);
    ...
    *id = sflashData;
	gk7101_spinand_release_channel(info);
    return 0;
 ...
 */
static int gk7101_spinand_reset(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;
    ...
    //LindengYu: Don't change the speed, use the sflash config
    ...
    gk7101_spinand_read_id(chip, &sflashData);
...
#if defined(KE_DEBUG_R) || defined(KE_DEBUG_W)
{
    ...
    gk7101_spinand_get_feature(chip, REG_PROTECTION, &proData);
    gk7101_spinand_get_feature(chip, REG_FEATURE, &feaData);
    gk7101_spinand_get_feature(chip, REG_STATUS, &staData);
...
/**
 * gk7101_spinand_unlock_allblock- send write register 0x1f command to the Nand device
 *
 * Description:
 *    After power up, all the Nand blocks are locked.  This function allows
 *    one to unlock the blocks, and so it can be wriiten or erased.
 */
static int gk7101_spinand_unlock_allblock(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;

    gk7101_spinand_set_channel(info);
    gk7101_spinand_set_feature(chip , REG_PROTECTION, BL_ALL_UNLOCKED);
    gk7101_spinand_release_channel(info);
    return 0;
}
...
/**
 * gk7101_spinand_enable_bbi - send command 0x1f to write the SPI Nand OTP register
 *
 * Description:
 ...
 */
static int gk7101_spinand_enable_bbi(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;
    uint32_t ufeature = 0;
    ...
    gk7101_spinand_get_feature(chip, REG_FEATURE, &ufeature);
    ...
    else
    {
        ...
        gk7101_spinand_set_feature(chip, REG_FEATURE, (uint8_t)(ufeature & 0xff));
        gk7101_spinand_get_feature(chip, REG_FEATURE, &ufeature);
        return ufeature;
    }
}

static int gk7101_spinand_disable_bbi(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;
    uint32_t ufeature = 0;
...
/**
 * gk7101_spinand_enable_ecc - send command 0x1f to write the SPI Nand OTP register
 *
 * Description:
 *   There is one bit( bit 0x10 ) to set or to clear the internal ECC.
 ...
 */
static int gk7101_spinand_enable_ecc(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;
    uint32_t ufeature = 0;
...
static int gk7101_spinand_disable_ecc(struct spinand_chip *chip)
{
    struct spinand_info *info = chip->info;
    uint32_t ufeature = 0;
...
static int gk7101_spinand_read_page_to_cache(struct spinand_chip *chip, uint32_t page_id)
{
    struct spinand_info *info = chip->info;
 ...
 *   No tRd delay.
 */
static int gk7101_spinand_read_from_cache(
 ...
 */
static int gk7101_spinand_read_page_raw(
            struct spinand_chip *chip,
            ...
            uint32_t len, uint8_t* rbuf)
{
    ...
    gk7101_spinand_disable_ecc(chip);

    gk7101_spinand_set_channel(info);

    retval = gk7101_spinand_read_page_to_cache(chip, page_id);

    while (1)
    {
        gk7101_spinand_get_feature(chip, REG_STATUS, &status);
    ...
    retval = gk7101_spinand_read_from_cache(chip, offset, len, rbuf);

    gk7101_spinand_release_channel(info);

    gk7101_spinand_enable_ecc(chip);
 ...
 *   Poll to read status to wait for tRD time.
 */
static int gk7101_spinand_read_page(
            struct spinand_chip *chip,
            uint32_t page_id, uint32_t offset,
            uint32_t len, uint8_t* rbuf)
{
...
static int gk7101_spinand_write_to_cache(
...
/**
 * gk7101_spinand_program_data_to_cache -- to write a page to cache with:
 ...
 */
static int gk7101_spinand_program_data_to_cache(
            ...
            uint32_t len, uint8_t* wbuf)
{
    ...
    {
        dflag = WRITE_CLWORD;
        gk7101_spinand_write_to_cache(chip, offset, dflag);
        for(i = 0; i < u4Len; i++)
        {
            uTmpData = *pData++;
    ...
    while(uleftLen)
    {
        ...
        gk7101_spinand_write_to_cache(chip, (offset + doff), dflag);
...
/**
 * gk7101_spinand_program_execute -- to write a page from cache to the Nand array with:
 ...
 */
static int gk7101_spinand_program_execute(struct spinand_chip *chip, uint32_t page_id)
{
    struct spinand_info *info = chip->info;
...
/**
 * gk7101_spinand_program_page_raw--to write a page with:
 * @page_id: the physical page location to write the page.
 ...
 */
static int gk7101_spinand_program_page_raw(
            struct spinand_chip *chip,
            uint32_t page_id, uint32_t offset,
            uint32_t len, uint8_t* wbuf)
{
    ...
    retval = gk7101_spinand_write_enable(chip, WR_ENABLE);

    retval = gk7101_spinand_program_data_to_cache(chip, offset, len, wbuf);

    retval = gk7101_spinand_program_execute(chip, page_id);
 ...
 *   Poll to wait for the tPROG time to finish the transaction.
 */
static int gk7101_spinand_program_page(
            struct spinand_chip *chip,
            uint32_t page_id, uint32_t offset,
            uint32_t len, uint8_t* wbuf)
{
 ...
 */
static int gk7101_spinand_erase_block_do(struct spinand_chip *chip, uint32_t block_id)
{
    struct spinand_info *info = chip->info;
 ...
 */
static int gk7101_spinand_erase_block(struct spinand_chip *chip, uint32_t block_id)
{
    struct spinand_info *info = chip->info;
    ...
    retval = gk7101_spinand_erase_block_do(chip, block_id);
...
/*                                                                                            */
/**************************************************************/
static struct spinand_info* gk7101_get_spinand_info(void)
{
    uint32_t sflashData;
    uint8_t  mID;
    uint16_t dID;
    ...
    //printf("SPINAND MID = 0x%x, DID = 0x%x, Data = 0x%x !",
    //            mID, dID, sflashData);
...
int spinand_board_init(struct mtd_info *mtd)
{
    ...
    print_size(info->nand_size," ");
    printf("(%s) ", info->deviceName);
#endif

    chip->info = info;

    chip->reset             = gk7101_spinand_reset;
    chip->read_id           = gk7101_spinand_read_id;
    chip->read_page         = gk7101_spinand_read_page;
    chip->read_page_raw     = gk7101_spinand_read_page_raw;
    chip->program_page      = gk7101_spinand_program_page;
    chip->program_page_raw  = gk7101_spinand_program_page_raw;
    chip->erase_block       = gk7101_spinand_erase_block;
    chip->scan_bbt          = NULL;
    ...
    chip->oobbuf = malloc((info->page_size + 7));
    if (!chip->oobbuf)
    {
    ...
    res = gk7101_spinand_unlock_allblock(chip);
    res = gk7101_spinand_enable_ecc(chip);
