...
int gk7101_gpio_func_config(u32 gpioid, u32 gtype)
{
    int             retval = 0;
...
void GH_SDIO_set_Control01Reg_SoftwareResetAll(u8 data)
{
    ...
    d.all = gk_sd_readl(REG_SDIO_CLK_CONTROL);
    d.bitc.softwareresetall = data;
    gk_sd_writel(REG_SDIO_CLK_CONTROL,d.all);
}
...
{
    u32 value = gk_sd_readl(REG_SDIO_CLK_CONTROL);
...
void GH_SDIO_set_IntStatusReg(u32 data)
{
    gk_sd_writel(REG_SDIO_NOR_INT_SIG_EN,data);
}

u32  GH_SDIO_get_IntStatusReg(void)
{
    u32 value = gk_sd_readl(REG_SDIO_NOR_INT_SIG_EN);

    return value;
}

void GH_SDIO_set_IntStatusEnReg(u32 data)
{
    gk_sd_writel(REG_SDIO_NOR_INT_STA_EN,data);
}

void GH_SDIO_set_IntSigEnReg(u32 data)
{
    gk_sd_writel(REG_SDIO_NOR_INT_SIG_EN,data);
}
...
{
    /*clear int*/
    if(GH_SDIO_get_IntStatusReg() & (0x1 << 15))
    {
        GH_SDIO_set_IntStatusReg(GH_SDIO_get_IntStatusReg());
    }

    GH_SDIO_set_IntStatusEnReg(0x0);
    GH_SDIO_set_IntStatusEnReg( SDIO_INT_STATUS_EN | 0xffff0000);
}

static void sdioEnableIntSig(void)
{
    GH_SDIO_set_IntSigEnReg(0);
    GH_SDIO_set_IntSigEnReg( SDIO_INT_SIG_EN | 0xffff0000);
}
...
void gk710x_save_gpio_cfg_to_usr_memory(u32 usrMemory)
{
    int i;
	int size;
    char *s;
...
void gk7101_phy_gpio_initialize()
{
	int i;
	int size;
	int phy_type = 0;
