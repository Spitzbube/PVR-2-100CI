...
int osd_function_show_date_v2(GADI_SYS_HandleT osdHandle,
                                  unsigned short encodeWidth,
                                  unsigned short encodeHeight,
                                  int planeId,
                                  int areaId,
                                  OSD_TextParamsT *text)
{
	...
	if((osdBitMap[planeId * OSD_AREA_NUM + areaId]) ==NULL)	
	{
		osdBitMap[planeId * OSD_AREA_NUM + areaId] =calloc(sizeof(RGBQUAD), osdText.dataLen);
