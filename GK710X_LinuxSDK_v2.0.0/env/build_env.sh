...
export CROSS_COMPILE=arm-goke-linux-gnueabi-
else
export CROSS_COMPILE=arm-goke-linux-uclibcgnueabi-
fi
ARM_LINUX_TOOLCHAIN_NAME=`echo $CROSS_COMPILE | sed 's/-$//'`

if [  ! -d $TOOLCHAIN_PARENT_DIRECTORY/ct_$C_LIBRARY ]; then
echo -e "\033[31m can't find tooltain in following dir!\033[0m"
echo "$TOOLCHAIN_PARENT_DIRECTORY/ct_$C_LIBRARY"
echo -e "\033[31m Please do command 'make deploy-tooltain' to install, then try again!\033[0m"
cd - > /dev/null
return 1
fi

if [  ! -d $ROOTFS_PARENT_DIRECTORY/rootfs_$C_LIBRARY ]; then
echo -e "\033[31m can't find rootfs in following dir!\033[0m"
echo "$ROOTFS_PARENT_DIRECTORY/rootfs_$C_LIBRARY"
echo -e "\033[31m Please do command 'make deploy-rootfs' to install, then try again!\033[0m"
cd - > /dev/null
return 1
fi
...
export TOOLCHAIN_TOP=$TOOLCHAIN_PARENT_DIRECTORY/ct_$C_LIBRARY
if [ 0 -eq `echo $PATH | grep -c $TOOLCHAIN_PARENT_DIRECTORY/ct_glibc/$GCC_VER/usr/bin` ]; then
export PATH=$PATH:$TOOLCHAIN_PARENT_DIRECTORY/ct_glibc/$GCC_VER/usr/bin
fi
if [ 0 -eq `echo $PATH | grep -c $TOOLCHAIN_PARENT_DIRECTORY/ct_uClibc/$GCC_VER/usr/bin` ]; then
export PATH=$PATH:$TOOLCHAIN_PARENT_DIRECTORY/ct_uClibc/$GCC_VER/usr/bin
fi
