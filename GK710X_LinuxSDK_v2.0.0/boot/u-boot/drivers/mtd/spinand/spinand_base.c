/*
 *  linux/drivers/mtd/spinand/spinand_base.c
 *
                    ...
                    loff_t ofs, uint64_t len)
{
    ...
    if (len & ((1 << chip->phys_erase_shift) - 1)) {
        printk("%s: length not block aligned\n", __func__);
        ret = -EINVAL;
    }

    /* Do not allow past end of device */
    if (ofs + len > mtd->size) {
        printk("%s: Past end of device\n", __func__);
        ret = -EINVAL;
    }
                ...
                struct mtd_oob_ops *ops)
{
    ...
    if (unlikely(ops->ooboffs >= ooblen)) {
        printk("%s: attempt to start read outside oob\n", __func__);
        return -EINVAL;
    }
    ...
    if (unlikely(from >= mtd->size ||
        ...
        printk("%s: attempt to read beyond end of device\n", __func__);
        return -EINVAL;
    }
    ...
    while (1)
    {
        ...
        if (count < oob_num)
        {
            ...
            {
                retval = chip->read_page_raw(chip, (page_id + count), info->page_main_size, info->page_spare_size, chip->oob_poi);
            }
            else
            {
                retval = chip->read_page(chip, (page_id + count), info->page_main_size, info->page_spare_size, chip->oob_poi);
            }
            if (retval != 0)
            {
                printk("%s: fail, block = %d, PageId = 0x%08x\n", __FUNCTION__,
                 ...
                 struct mtd_oob_ops *ops)
{
    ...
    #endif
    printk(KERN_INFO "%s: to = 0x%08x, len = %i\n", __func__, (unsigned int)to, (int)ops->ooblen);
    ...
    if ((ops->ooboffs + ops->ooblen) > ooblen) {
        printk("%s: attempt to write past end of page\n", __func__);
        return -EINVAL;
    }

    if (unlikely(ops->ooboffs >= ooblen)) {
        printk("%s: attempt to start write outside oob\n", __func__);
        return -EINVAL;
    }
    ...
    if (unlikely(to >= mtd->size ||
        ...
        printk("%s: attempt to write beyond end of device\n", __func__);
        return -EINVAL;
    }
...
static int spinand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
    ...
    default:
        printk("%s: The mode = %d no supported\n", __func__, ops->mode);
        goto out;
    }
...
int spinand_erase_nand(struct mtd_info *mtd, struct erase_info *instr, int allowbbt)
{
    ...
    while (len)
    {
        ...
        if (!instr->scrub && spinand_block_checkbad(mtd, ((loff_t) page) << chip->page_shift, allowbbt))
        {
...
erase_exit:
    ...
    if (rewrite_bbt[0])
    {
        ...
        printk("%s: update_bbt (0x%0llx 0x%08x)\n",
...
static int spinand_block_bad(struct mtd_info *mtd, loff_t ofs)
{
    ...
    #ifdef KE_DEBUG_R
        printk("[%s]: page_id = 0x%08x, Ofs = 0x%08x\n",
...
static int spinand_default_block_bad(struct mtd_info *mtd, loff_t ofs)
{
    ...
    #ifdef KE_DEBUG_R
    printk("[%s]: Block = 0x%04x, is_bad = 0x%x\n",
                __FUNCTION__, (uint32_t)(ofs >> chip->bbt_erase_shift), is_bad);
    #endif
...
static int spinand_default_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
    ...
    else
    {
        ...
        do {
            ...
            ret = spinand_do_write_oob(mtd, ofs, &ops);

            if (!ret && (chip->options & NAND_BBT_SCANBYTE1AND6)) {
                ops.ooboffs = NAND_SMALL_BADBLOCK_POS & ~0x01;
                ret = spinand_do_write_oob(mtd, ofs, &ops);
            }
            i++;
            ofs += mtd->writesize;
        } while (!ret && (chip->options & NAND_BBT_SCAN2NDPAGE) && i < 2);
...
/**
 * spinand_release - [SpiNAND Interface] Free resources held by the SpiNAND device
 * @param mtd       MTD device structure
 */
void spinand_release(struct mtd_info *mtd)
{
    return;
}
