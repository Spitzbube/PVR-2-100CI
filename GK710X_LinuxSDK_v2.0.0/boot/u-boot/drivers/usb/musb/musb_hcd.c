...
static const struct musb_epinfo epinfo[3] = {
	{MUSB_BULK_EP_OUT, 1, 512}, /* EP1 - Bluk Out - 512 Bytes */
	{MUSB_BULK_EP_IN,  0, 512}, /* EP1 - Bluk In  - 512 Bytes */
...
static int ctrlreq_out_data_phase(struct usb_device *dev, u32 len, void *buffer)
{
	...
	while (txlen < len) {
		...
		/* Set TXPKTRDY bit */
		csr = musb_readw(&musbr->txcsr);
		musb_writew(csr | MUSB_CSR0_H_DIS_PING | MUSB_CSR0_TXPKTRDY,
					&musbr->txcsr);
		result = wait_until_ep0_ready(dev, MUSB_CSR0_TXPKTRDY);
		if (result < 0)
			break;
...
static void config_hub_port(struct usb_device *dev, u8 ep)
{
	...
	/* configure the hub address and the port address */
	musb_writeb(hub, &musbr->tar[ep].txhubaddr);
	musb_writeb((chid + 1), &musbr->tar[ep].txhubport);
	musb_writeb(hub, &musbr->tar[ep].rxhubaddr);
	musb_writeb((chid + 1), &musbr->tar[ep].rxhubport);
#endif
}
...
{
	u8 power = musb_readb(&musbr->power);

	if (do_reset) {
		power &= 0xf0;
		musb_writeb(power | MUSB_POWER_RESET, &musbr->power);
			      ...
			      struct devrequest *cmd)
{
	...
	case RH_GET_STATUS | RH_OTHER | RH_CLASS:
		...
		int_usb = musb_readw(&musbr->intrusb);
			...
			int len, struct devrequest *setup)
{
	...
	musb_writeb(MUSB_CONTROL_EP, &musbr->index);
	musb_readw(&musbr->txcsr);

#ifndef MUSB_NO_MULTIPOINT
	/* target addr and (for multipoint) hub addr/port */
	musb_writeb(devnum, &musbr->tar[MUSB_CONTROL_EP].txfuncaddr);
	musb_writeb(devnum, &musbr->tar[MUSB_CONTROL_EP].rxfuncaddr);
#endif
	...
	if ((musb_ishighspeed()) && (dev->parent != NULL) &&
		...
		musb_writeb(devspeed << 6, &musbr->txtype);
	} else {
		musb_writeb(musb_cfg.musb_speed << 6, &musbr->txtype);
#ifndef MUSB_NO_MULTIPOINT
		musb_writeb(0, &musbr->tar[MUSB_CONTROL_EP].txhubaddr);
		musb_writeb(0, &musbr->tar[MUSB_CONTROL_EP].txhubport);
		musb_writeb(0, &musbr->tar[MUSB_CONTROL_EP].rxhubaddr);
		musb_writeb(0, &musbr->tar[MUSB_CONTROL_EP].rxhubport);
#endif
	}
	...
	if (setup->request == USB_REQ_SET_ADDRESS)
		musb_writeb(dev->devnum, &musbr->faddr);
#endif
 ...
 * do a control transfer
 */
 //#define control_msb_printf(fmt, args...) printf(fmt,##args)
#define control_msb_printf(fmt, args...) 

int submit_control_msg_asix(struct usb_device *dev, unsigned long pipe, void *buffer,
			int len, struct devrequest *setup)
{
	int devnum = usb_pipedevice(pipe);
	u8  devspeed;
    ...
    control_msb_printf("setup->request-1:%d\n",setup->request);
	/* Control transfer setup phase */
	if (ctrlreq_setup_phase(dev, setup) < 0)
		return 0;
    control_msb_printf("setup->request-2:%d\n",setup->request);
	...
	case AX_CMD_READ_PHY_ID:
        control_msb_printf("setup->request: read in-data-phase\n");
		/* control transfer in-data-phase */
		if (ctrlreq_in_data_phase(dev, len, buffer) < 0)
			return 0;
		/* control transfer out-status-phase */
        control_msb_printf("setup->request: read out_status_phase\n");
		if (ctrlreq_out_status_phase(dev) < 0)
			return 0;
		break;
	...
	case AX_CMD_WRITE_MII_REG:
        ...
        int csr;
        		/* control transfer out data phase */
        control_msb_printf("setup->request: 2 write out_data_phase\n");
		if (ctrlreq_out_data_phase(dev, len, buffer) < 0)
			return 0;
	}
        break;
    ...
    case AX_CMD_WRITE_NODE_ID:        
		/* control transfer out data phase */
        control_msb_printf("setup->request: write out_data_phase,len:%d\n",len);
		if (ctrlreq_out_data_phase(dev, len, buffer) < 0)
			return 0;
		/* control transfer in status phase */
        control_msb_printf("setup->request: write in_status_phase\n");
		if (ctrlreq_in_status_phase(dev) < 0)
		{
            control_msb_printf("setup->request: write in_status_phase end 1\n");
            return 0;
		}
        control_msb_printf("setup->request: write in_status_phase end 2\n");
        break;


	default:
		/* unhandled control transfer */
        printf("submit_control error:%d\n",setup->request);
		return -1;
	}
					...
					void *buffer, int len)
{
    ...
    if(dir_out)
    {
        bulk_ep = MUSB_BULK_EP_OUT;
    }
    else
    {
        bulk_ep = MUSB_BULK_EP_IN; 
    }
    
	/* select bulk endpoint */
	musb_writeb(bulk_ep, &musbr->index);

#ifndef MUSB_NO_MULTIPOINT
	/* write the address of the device */
	if (dir_out)
		musb_writeb(devnum, &musbr->tar[bulk_ep].txfuncaddr);
	else
		musb_writeb(devnum, &musbr->tar[bulk_ep].rxfuncaddr);
#endif
	...
	if ((musb_ishighspeed()) && (dev->parent != NULL) &&
		 ...
		 * address registers.
		 */
		config_hub_port(dev, bulk_ep);
	} else {
#ifndef MUSB_NO_MULTIPOINT
		if (dir_out) {
			musb_writeb(0, &musbr->tar[bulk_ep].txhubaddr);
			musb_writeb(0, &musbr->tar[bulk_ep].txhubport);
		} else {
			musb_writeb(0, &musbr->tar[bulk_ep].rxhubaddr);
			musb_writeb(0, &musbr->tar[bulk_ep].rxhubport);
		}
#endif
	...
	if (dir_out) { /* bulk-out transfer */
		...
		musb_writeb(type, &musbr->txtype);

		/* Write maximum packet size to the TxMaxp register */
		musb_writew(dev->epmaxpacketout[ep], &musbr->txmaxp);       
		while (txlen < len) {
			...
			musb_writew(nextlen, &musbr->txcount);
#endif
			/* Write the data to the FIFO */
			write_fifo(bulk_ep, nextlen,
					(void *)(((u8 *)buffer) + txlen));
            
            /* Set the TxPktRdy bit */
			csr = musb_readw(&musbr->txcsr);
			musb_writew(csr | MUSB_TXCSR_TXPKTRDY, &musbr->txcsr);
                       
			/* Wait until the TxPktRdy bit is cleared */
			if (!wait_until_txep_ready(dev, bulk_ep)) {
				musb_readw(&musbr->txcsr);
		...
		type = (devspeed << MUSB_TYPE_SPEED_SHIFT) |
			   (MUSB_TYPE_PROTO_BULK << MUSB_TYPE_PROTO_SHIFT) |
			   (ep & MUSB_TYPE_REMOTE_END);
		musb_writeb(type, &musbr->rxtype);

		/* Write the maximum packet size to the RxMaxp register */
		musb_writew(dev->epmaxpacketin[ep], &musbr->rxmaxp);

        csr = musb_readw(&musbr->rxcsr);
        if((csr | MUSB_RXCSR_H_REQPKT) == 0)
        {
            csr = csr | MUSB_RXCSR_H_REQPKT;
        }
        csr = csr | MUSB_RXCSR_H_AUTOREQ;
        musb_writew(csr , &musbr->rxcsr);
		while (txlen < len) {
            ...
            result = wait_until_rxep_ready(dev, bulk_ep);
            ...
            else if(result == -1 && timeOutFirst != 0) // timeout
            {
                len = txlen ;
                break;
            }
            ...
            rxcount = musb_readw(&musbr->rxcount);
            rxcount &= 0x1fff;
            if(rxcount > nextlen)
            {
                len = txlen ;
...
int usb_lowlevel_init(void)
{
	...
	while (--timeout)
		if (musb_readb(&musbr->devctl) & MUSB_DEVCTL_HM)
			break;
	...
	musb_writeb(power, &musbr->power);

	/* Determine if the connected device is a high/full/low speed device */
	musb_cfg.musb_speed = (musb_readb(&musbr->power) & MUSB_POWER_HSMODE) ?
			MUSB_TYPE_SPEED_HIGH :
			((musb_readb(&musbr->devctl) & MUSB_DEVCTL_FSDEV) ?
			MUSB_TYPE_SPEED_FULL: MUSB_TYPE_SPEED_LOW);
    printf("usb speed:%d\n",musb_cfg.musb_speed);
    musb_hcd_set_speed(musb_cfg.musb_speed);
	return 0;
}
				...
				void *buffer, int len, int interval)
{
	...
	musb_writeb(MUSB_INTR_EP, &musbr->index);

#ifndef MUSB_NO_MULTIPOINT
	/* write the address of the device */
	if (dir_out)
		musb_writeb(devnum, &musbr->tar[MUSB_INTR_EP].txfuncaddr);
	else
		musb_writeb(devnum, &musbr->tar[MUSB_INTR_EP].rxfuncaddr);
#endif
...
#ifndef MUSB_NO_MULTIPOINT
		if (dir_out) {
			musb_writeb(0, &musbr->tar[MUSB_INTR_EP].txhubaddr);
			musb_writeb(0, &musbr->tar[MUSB_INTR_EP].txhubport);
		} else {
			musb_writeb(0, &musbr->tar[MUSB_INTR_EP].rxhubaddr);
			musb_writeb(0, &musbr->tar[MUSB_INTR_EP].rxhubport);
		}
#endif
		devspeed = musb_cfg.musb_speed;
	}
	...
	if (!dir_out) { /* intrrupt-in transfer */
		/* Write the saved toggle bit value */
		write_toggle(dev, ep, dir_out);
		musb_writeb(interval, &musbr->rxinterval);
		...
		while (txlen < len) {
			...
			/* Set the ReqPkt bit */
			csr = musb_readw(&musbr->rxcsr);
			musb_writew(csr | MUSB_RXCSR_H_REQPKT, &musbr->rxcsr);
