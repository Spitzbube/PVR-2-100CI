/*!
*****************************************************************************
** \file        jxh42_reg_tbl.c
**
** \version     $Id: jxh42_reg_tbl.c 10611 2016-10-14 09:26:10Z yulindeng $
**
** \brief
**
...
*****************************************************************************
*/
GADI_VI_SensorDrvInfoT    jxh42_sensor =
{
    ...
    .field_format       = 1,
    .adapter_id         = 0,
    .sensor_id          = GADI_ISP_SENSOR_JXH42,
    ...
    .init_reg           =
    {
        ...
        //{JXH42_SYS,       0x40},  // 0x12
        // DVP Setting
        {0x0C, 0x00},  // 0x00 JXH42_DVP
        {0x0D, 0x5C},  // 0x40 JXH42_DVP2
        {0x1F, 0x04},  // 0x1F JXH42_DVP5
        // PLL Setting
        //{0x0E,      0x1D},  // 0x0E JXH42_PLL1
        //{0x0F,      0x09},  // 0x0F JXH42_PLL2
        //{0x10,      0x1E},  // 0x10 JXH42_PLL3
        //{0x11,      0x80},  // 0x11 JXH42_CLK
        // Frame/Window
        {0x2A, 0x24},
        {0x2B, 0x29},
        {0x2C, 0x04},  // 0x2C 03 JXH42_SENHAST
        {0x2D, 0x00},  // 0x2D 0x000=0 JXH42_SENVST
        {0x2E, 0xB9},  // 0x2E 0x0B9=185 JXH42_SENVEND
        {0x2F, 0x00},  // 0x2F JXH42_SENVADD
        // Sensor Timing
        {0x30, 0x92},
        {0x31, 0x0A},
        {0x32, 0xAA},
        ...
        {0x43, 0x50}, //40
        // Interface
        {0x1D, 0xFF},  // 0x1D JXH42_DVP3 port D[7:0] GPIO out
        {0x1E, 0x9F},  // 0x1E JXH42_DVP4 PCLK HREF VSYNC PCLK HREF VSYNC port D[9:8] GPIO out
        {0x6C, 0x90}, // JXH42_DPHY2
        {0x73, 0xB3},
        {0x70, 0x68},
        {0x76, 0x40}, // JXH42_MIPI7
        {0x77, 0x06}, // JXH42_MIPI8
        // Array/AnADC/PWC
        {0x48, 0x60},
        {0x60, 0xA4},
        {0x61, 0xFF},
        ...
        {0x6A, 0x09},
        // AE/AG/ABLC
        {0x13, 0x87},  // 0x13 JXH42_LCCTRL1Gain ceiling 100: 32x
        {0x14, 0x80},  // 0x14 JXH42_LCCTRL2
        {0x16, 0xC0},  // 0x16 JXH42_LCCTRL4
        {0x17, 0x40},  // 0x17 JXH42_LCCTRL5
        {0x18, 0xBB},  // 0x18 JXH42_LCCTRL6
        {0x38, 0x35},  // 0x38 JXH42_VAVEST
        {0x39, 0x98},  //  JXH42_VAVEWID
        {0x4A, 0x03},  // 0x4A JXH42_BLC_CTRL
        {0x49, 0x04},  // 0x49 JXH42_BLC_TGT
        // INI End
        // PWDN Setting
        //{0x12, 0x00}, // JXH42_SYS
        {GADI_VI_SENSOR_TABLE_FLAG_END,}
    ...
    .pll_table =
    {
        ...
        {
            // for 720P@30, 720P@25
            .pixclk = 37125000, // sysclk = PLLclk * (1 + CLK[6]) / 2
                                //        = [VCO / (1+PLL2[3:0])] * (1 + CLK[6]) / 2
                                //        = [(Input clock(extclk) * PLL3[7:0] / PLL_Pre_Ratio)/(1 + PLL2[3:0]) * (1 + CLK[6]) / 2
                                //        = [(Input clock(extclk) * PLL3[7:0] / (1 + PLL[1:0]))/(1 + PLL2[3:0]) * (1 + CLK[6]) / 2
            .extclk = 27000000, //        = (27000000 * 22 / (1 + 1)) / (1 + 7) * (1 + 0) / 2 = 36000000
            .regs =
            {
                {0x0E, 0x1D},   // JXH42_PLL1 PLL_Pre_Ratio = (1 + PLL[1:0]) = 2
                                // PLL1[7]: PLL bypass selection.
                                //    1: PLLclk = Input clock / PLL_Pre_Ratio.
                                //    0: see PLL2[3:0].
                                // PLL1[6:4]: reserved.
                                // PLL1[3]: PLL mipi clock and mipi pixel clock ratio selection,
                                //    0: ration 8:1,
                                //    1: ration 10:1.
                                // PLL1[2]: rsvd.
                                // PLL1[1:0]: PLL pre-divider ratio. PLL_Pre_Ratio = (1 + PLL[1:0])
                                // 0001 1101
                {0x0F, 0x07},   // JXH42_PLL2 Mipiclk = VCO/(1+PLL2[7:4])  PLLclk = VCO / (1+PLL2[3:0])
                {0x10, 0x16},   // JXH42_PLL3 PLL VCO = Input clock * PLL3[7:0] / PLL_Pre_Ratio
                {0x11, 0x80},   // JXH42_CLK When CLK[5:0] > 0: System clock = PLLclk * (1 + CLK[6]) / (2 * CLK[5:0])
    ...
    .video_fmt_table =
    {
        [0] =   // 1280x720P@30fps    1650 * 750 * 30 = 37125000
        {
            ...
            .fmt_reg =
            {
                {0x20, 0x72},   // LINE_LENGTH_L=JXH42_FRAMEW_L
                {0x21, 0x06},   // LINE_LENGTH_H=JXH42_FRAMEW_H     1650 = 0x0672
                {0x22, 0xEE},   // FRM_LENGTH_L=JXH42_FRAMEH_L
                {0x23, 0x02},   // FRM_LENGTH_H=JXH42_FRAMEH_H       750 = 0x02EE
                {0x27, 0x3B},   // X_ADD_STA_L=JXH42_HWINST       // must >1600-1296=0x130 >0x13B < 1600-1280=0x140<0x14B
                {0x28, 0x0D},   // Y_ADD_STA_L=JXH42_VWINST       // 750-720=0x1E
                {0x29, 0x01},   // Y/X_ADD_STA_H=JXH42_HVWINST
                {0x24, 0x00},   // X_OUT_SIZE_L=JXH42_HWIN          1280 = 0x0500
                {0x25, 0xD0},   // X_OUT_SIZE_H=JXH42_VWIN           720 = 0x02D0
                {0x26, 0x25},   // Y/X_OUT_SIZE_L=JXH42_HVWIN
                {0x18, 0xBB},   //
                {GADI_VI_SENSOR_TABLE_FLAG_END,},
            },
            .vi_info =
        ...
        [1] =   // 1280x720P@25fps    1980 * 750 * 25 = 37125000
        {
            ...
            .fmt_reg =
            {
                {0x20, 0xBC},   // LINE_LENGTH_L=JXH42_FRAMEW_L
                {0x21, 0x07},   // LINE_LENGTH_H=JXH42_FRAMEW_H     1980 = 0x07BC
                {0x22, 0xEE},   // FRM_LENGTH_L=JXH42_FRAMEH_L
                {0x23, 0x02},   // FRM_LENGTH_H=JXH42_FRAMEH_H       750 = 0x02EE
                {0x27, 0x3B},   // X_ADD_STA_L=JXH42_HWINST       // must >1600-1296=0x130 >0x13B < 1600-1280=0x140<0x14B
                {0x28, 0x0D},   // Y_ADD_STA_L=JXH42_VWINST       // 750-720=0x1E
...
#define JXH42_H_MIRROR  (1<<5)
    ...
    // hmax = (JXH42_FRAMEW_H & 0x3F) << 8 +
    //        (JXH42_FRAMEW_L & 0xFF) << 0
    .hmax_reg =
    {
        {0x21, 0x00, 0x3F, 0x00, 0x08, 0}, // JXH42_FRAMEW_H
        {0x20, 0x00, 0xFF, 0x00, 0x00, 0}, // JXH42_FRAMEW_L
        {GADI_VI_SENSOR_TABLE_FLAG_END,},
    },
    // vmax = (JXH42_FRAMEH_H & 0xFF) << 8 +
    //        (JXH42_FRAMEH_L & 0xFF) << 0
    .vmax_reg =
    {
        {0x23, 0x00, 0xFF, 0x00, 0x08, 0}, // JXH42_FRAMEH_H
        {0x22, 0x00, 0xFF, 0x00, 0x00, 0}, // JXH42_FRAMEH_L
        {GADI_VI_SENSOR_TABLE_FLAG_END,},
    },
    .vmax_wadd_reg = {GADI_VI_SENSOR_TABLE_FLAG_END,},
    // shs = (JXH42_EXP_H & 0xFF) << 8 +
    //       (JXH42_EXP_L & 0xFF) << 0
    .shs_reg =
    {
        {0x02, 0x00, 0xFF, 0x00, 0x08, 0}, // JXH42_EXP_H
        {0x01, 0x00, 0xFF, 0x00, 0x00, 0}, // JXH42_EXP_L
        {GADI_VI_SENSOR_TABLE_FLAG_END,},
    },
