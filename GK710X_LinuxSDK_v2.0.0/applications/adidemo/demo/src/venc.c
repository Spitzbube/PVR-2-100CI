...
//*****************************************************************************
//*****************************************************************************
static const char *vencShortOptions = "hb:f:g:I:S:P:q:Q:l:T:r:F:c:B:";
...
{
    {
        ...
        {"h264_adapt_qp",     &streams[0].h264Conf.adaptQp,        DATA_TYPE_U8},
        {"h264_qp_min_I",     &streams[0].h264Conf.qpMinI,         DATA_TYPE_U8},
        {"h264_qp_min_P",     &streams[0].h264Conf.qpMinP,         DATA_TYPE_U8},
        {"h264_qpIWeight",    &streams[0].h264Conf.qpIWeight,      DATA_TYPE_U8},
        {"h264_qpPWeight",    &streams[0].h264Conf.qpPWeight,      DATA_TYPE_U8},
    ...
    },
    {
        ...
        {"h264_adapt_qp",     &streams[1].h264Conf.adaptQp,        DATA_TYPE_U8},
        {"h264_qp_min_I",     &streams[1].h264Conf.qpMinI,         DATA_TYPE_U8},
        {"h264_qp_min_P",     &streams[1].h264Conf.qpMinP,         DATA_TYPE_U8},
        {"h264_qpIWeight",    &streams[1].h264Conf.qpIWeight,      DATA_TYPE_U8},
        {"h264_qpPWeight",    &streams[1].h264Conf.qpPWeight,      DATA_TYPE_U8},
    ...
    },
    {
        ...
        {"h264_adapt_qp",     &streams[2].h264Conf.adaptQp,        DATA_TYPE_U8},
        {"h264_qp_min_I",     &streams[2].h264Conf.qpMinI,         DATA_TYPE_U8},
        {"h264_qp_min_P",     &streams[2].h264Conf.qpMinP,         DATA_TYPE_U8},
        {"h264_qpIWeight",    &streams[2].h264Conf.qpIWeight,      DATA_TYPE_U8},
        {"h264_qpPWeight",    &streams[2].h264Conf.qpPWeight,      DATA_TYPE_U8},
    ...
    },
    {
        ...
        {"h264_adapt_qp",     &streams[3].h264Conf.adaptQp,        DATA_TYPE_U8},
        {"h264_qp_min_I",     &streams[3].h264Conf.qpMinI,         DATA_TYPE_U8},
        {"h264_qp_min_P",     &streams[3].h264Conf.qpMinP,         DATA_TYPE_U8},
        {"h264_qpIWeight",    &streams[3].h264Conf.qpIWeight,      DATA_TYPE_U8},
        {"h264_qpPWeight",    &streams[3].h264Conf.qpPWeight,      DATA_TYPE_U8},
...
static GADI_BOOL    used_four_stream = GADI_FALSE;
...
static void video_test(GADI_U32 operaNum)
{
	...
	if(operaNum == 0){
		...
		if (retVal != GADI_OK) {
			...
			GADI_INFO("gadi_vi_get_resolution ok!\n");
...
GADI_ERR gdm_venc_open(void)
{
    ...
    GADI_INFO("gadi_venc_map_bsb ok\n");
    ...
    GADI_INFO("gadi_venc_map_dsp OK\n");
...
GADI_ERR gdm_venc_parse_config_file(char *path)
{
    ...
    for(i = 0; i < 4; i++)
    {
        GADI_INFO("streamFormat.streamId:%d,encodeType:%d,width:%d,height:%d,Fps:%d,flipRotate:%d,keepAspRat:%d,h264Conf.streamId:%d,gopModel:%d,gopM:%d,gopN:%d\n",
        ...
        GADI_INFO("idrInterval:%d,profile:%d,brcMode:%d,cbrAvgBps:%d,vbrMinbps:%d,vbrMaxbps:%d,mjpegConf.streamId:%d,quality:%d,chromaFormat:%d\n",
...
GADI_ERR gdm_venc_setup(void)
{
	...
	GADI_INFO("gadi_venc_set_channels_params ok!\n");

    /* if stream four is no used, use to capture picture. */
    if(streams[GADI_VENC_STREAM_FORTH].streamFormat.encodeType == 0) {
        _snapshot_init();
        used_four_stream = GADI_TRUE;
    }
    ...
    for (cnt = 0; cnt < GADI_VENC_STREAM_NUM; cnt++)
    {
        ...
        if(encodeType != 0)
        {
            ...
            GADI_INFO("gadi_venc_set_stream_format ok!\n");
            if(encodeType == GADI_VENC_TYPE_H264)
            {
                ...
                GADI_INFO("gadi_venc_set_h264_config ok!\n");
            }
            else if(encodeType == GADI_VENC_TYPE_MJPEG)
            {
                ...
                GADI_INFO("gadi_venc_set_mjpeg_config ok!\n");
            }
        }
    }
...
GADI_ERR gdm_venc_set_gopn(GADI_VENC_H264GopNConfigT *h264GopNconfig)
{
    GADI_ERR retVal = GADI_OK;

    if(h264GopNconfig->streamId >= GADI_VENC_STREAM_NUM){
        return GADI_ERR_BAD_PARAMETER;
    }

    retVal = gadi_venc_set_h264_gopn(vencHandle, h264GopNconfig);
    if (retVal != GADI_OK) {
        GADI_ERROR("gadi_venc_set_h264_gopn fail!\n");
        return retVal;
    }

    if(retVal == GADI_OK) {
		streams[h264GopNconfig->streamId].h264Conf.gopN = h264GopNconfig->gopN;
	}

    return retVal;
}

GADI_ERR gdm_venc_get_gopn(GADI_VENC_H264GopNConfigT *h264GopNconfig)
{
    GADI_ERR retVal = GADI_OK;
    ...
    retVal = gadi_venc_get_h264_gopn(vencHandle, h264GopNconfig);
    if (retVal != GADI_OK) {
        GADI_ERROR("gadi_venc_get_h264_gopn fail!\n");
        return retVal;
    }
...
static GADI_ERR handle_venc_command(int argc, char* argv[])
{
    ...
    while ((ch = getopt_long(argc, argv, vencShortOptions, vencLongOptions, &option_index)) != -1)
    {
		...
		case 'g':
        {
            GADI_VENC_H264GopNConfigT h264_gopn_config;

            stream_id = atoi(optarg);
            if(gdm_venc_get_gopn(&h264_gopn_config)) {
                break;
            }

            CREATE_INPUT_MENU(h264_gopn_config) {
                ADD_SUBMENU(h264_gopn_config.gopN,"[1-255]"),
                CREATE_INPUT_MENU_COMPLETE();
			...
			gdm_venc_set_gopn(&h264_gopn_config);
        ...
        case 'q':
        {
            ...
            CREATE_INPUT_MENU(h264_qp_config) {
                ...
                ADD_SUBMENU(h264_qp_config.adaptQp,"[0-4]quality consistency"),
                CREATE_INPUT_MENU_COMPLETE();
        ...
        case 'B':
        {
            ...
            GADI_VENC_EncConfigT enc_config = {0};
            ...
            enc_config.streamId = atoi(optarg);
            if(gadi_venc_get_h264_enc(vencHandle, &enc_config)) {
                GADI_ERROR("get enc param error!\n");
                break;
            }
            ...
            CREATE_INPUT_MENU(bias_config) {
                ADD_SUBMENU(bias_config.i_16x16_cost,   "[-64-64]i_16x16_cost"),
                ADD_SUBMENU(bias_config.i_4x4_cost,     "[-64-64]i_4x4_cost"),
                ADD_SUBMENU(bias_config.p_16x16_cost,   "[-64-64]p_16x16_cost"),
                ADD_SUBMENU(bias_config.p_8x8_cost,     "[-64-64]p_8x8_cost"),
                ADD_SUBMENU(bias_config.skip_chance,    "[1-51]skip_chance"),
                ADD_SUBMENU(bias_config.dbf_alpha,      "[-6-6]dbf_alpha"),
                ADD_SUBMENU(bias_config.dbf_beta,       "[-6-6]dbf_beta"),
                ADD_SUBMENU(enc_config.i_pskip_chance,  "[0-4000]i_pskip_chance"),
                ADD_SUBMENU(zmv_config.zero_mv_th,      "[0-255]zero_mv_th"),
                CREATE_INPUT_MENU_COMPLETE();
			...
			gadi_venc_set_h264_enc(vencHandle, &enc_config);
...
static void snapshot_process(void *params)
{
    ...
    do{
        if (gadi_venc_query(vencHandle, GADI_VENC_STREAM_FORTH, &chn_attr) < 0)
        {
            GADI_INFO("gadi_venc_query chn:%d", GADI_VENC_STREAM_FORTH);
            usleep(1000);
            continue;
        }
        ...
        {
            if (gadi_venc_get_stream(vencHandle, GADI_VENC_STREAM_FORTH, stream)<0)//BLOCK
            {
                continue;
            }
...
static GADI_VOID _snapshot_init(GADI_VOID)
{
    ...
    streams[GADI_VENC_STREAM_FORTH].streamFormat.width =
        streams[GADI_VENC_STREAM_FIRST].streamFormat.width;
    streams[GADI_VENC_STREAM_FORTH].streamFormat.height =
        streams[GADI_VENC_STREAM_FIRST].streamFormat.height;
    streams[GADI_VENC_STREAM_FORTH].streamFormat.xOffset =
        streams[GADI_VENC_STREAM_FIRST].streamFormat.xOffset;
    streams[GADI_VENC_STREAM_FORTH].streamFormat.yOffset =
        streams[GADI_VENC_STREAM_FIRST].streamFormat.yOffset;
    streams[GADI_VENC_STREAM_FORTH].streamFormat.channelId =
        streams[GADI_VENC_STREAM_FIRST].streamFormat.channelId;

    streamFormat = &streams[GADI_VENC_STREAM_FORTH].streamFormat;
    ...
    mjpegConf = &streams[GADI_VENC_STREAM_FORTH].mjpegConf;
    ...
    GADI_U8 quality, GADI_U16 width, GADI_U16 height)
{
    ...
    if(width >= streams[streamid].streamFormat.width){
        ...
        streams[GADI_VENC_STREAM_FORTH].streamFormat.width = width;
        streams[GADI_VENC_STREAM_FORTH].streamFormat.xOffset =
            streams[streamid].streamFormat.xOffset
    ...
    if(height >= streams[streamid].streamFormat.height){
        ...
        streams[GADI_VENC_STREAM_FORTH].streamFormat.height = height;
        streams[GADI_VENC_STREAM_FORTH].streamFormat.yOffset =
            streams[streamid].streamFormat.yOffset
                                    ...
                                    GAPP_ENC_SNAPSHOT_ImageSizeEnumT width, GAPP_ENC_SNAPSHOT_ImageSizeEnumT height, FILE* filename)
{
    ...
    /*start jpeg encoder.*/
    retVal = gadi_venc_start_stream(vencHandle, GADI_VENC_STREAM_FORTH);
    if(retVal < 0)
    {
