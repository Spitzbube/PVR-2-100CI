...
static int http_cgi_stream_control(HTTP_OPS* ops, void* arg)
{
    ...
    else
    {
        ...
        bps = cJosn_Read_Int(opt,"h264_cbrAvgBps");
        cJSON_Delete(opt);

        ret = video_set_bitRate(streamId, 0, bps);
        if(ret != 0)
        {
...
static char *json_stream_control_string(int id)
{
    ...
    offset += snprintf(jsonbuf+offset, MAX_JSON_LENGTH-offset, "\"h264_cbrAvgBpsProperty\": {\"type\":\"U32\",\"mode\":\"rw\",\"min\":0,\"max\":8192,\"def\":2000,\"opt\":\"\"}");
    offset += snprintf(jsonbuf+offset, MAX_JSON_LENGTH-offset, "}");
